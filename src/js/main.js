...
function mapApp() {
	...
	***********************************/
	...
	function appViewModel() {
		...
		self.placeClick = function(ID, isInitialLoad) {
			...
			if (alreadyMarker === false) {
				...
				yelpView.get_business(ID, function(result) { // create new marker via Yelp callback
					mapView.createPlaceMarker(result, isInitialLoad);
				});
			}
			else if (alreadyMarker === true) { // do not create marker, open infoWindow only
        ...
        currentMarker.infoWindowTrigger();
		...
		self.createInitialMarkers = function() {
          ...
          // 2nd parameter used to indicate initial load and prevent infoWindows from loading
					self.placeClick(self.viewModelFavourites()[i].key, true);
				}
			}
		};
	...
	***********************************/
    ...
    // isInitialLoad is boolean - determines whether infoWindow shown
		createPlaceMarker: function(place, isInitialLoad) {
      ...
      // listen to clicks to market itself, load infoWindow
      google.maps.event.addListener(self.marker, 'click', function() {
        this.infoWindowTrigger();
      });
      ...
      // load infoWindow on click IF not initial load of markers from favourites
      if (!isInitialLoad) {
        self.marker.infoWindowTrigger();
      }
		},
